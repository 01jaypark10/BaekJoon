...
//해당 d방향에 있는 칸이 청소가 안되어 있는 땅인지 참 거짓 리턴
int CheckIfIsCleaned(int arr[50][50],int r, int c, int d)
{
    switch (d)
    {
    case 0:
...
//해당 d방향으로 이동할 수 있는지 유무를 리턴
int CheckIfCanMove(int arr[50][50],int r, int c, int d)
{
    switch (d)
    {
    case 0:
...
{
    int N, M;   //세로, 가로 길이
    int room[50][50];   //방의 모습 0 : 청소되지 않은 곳, 1 : 벽이 있는 곳, -1 : 청소가 완료된 곳
    int r, c;   //로봇의 위치(r, c)   
    int d;  //로봇청소기가 바라보는 방향
    int count = 0;  //로봇청소기가 청소하는 칸의 개수
    ...
    while (1)
    {
        ...
        //주변 4칸 중에 아직 청소되어 있지 않은 칸이 있다면?
        if (room[r - 1][c] == 0 || room[r][c - 1] == 0 || room[r + 1][c] == 0 || room[r][c + 1] == 0)
        {
            d = (d + 3) % 4;
            
            if (CheckIfIsCleaned(room,r,c,d))
            {
                Move(&r,&c,d);
            }
        ...
        {
            if (CheckIfCanMove(room,r,c,(d + 2)%4))
            {
                Move(&r,&c,(d+2)%4);
            }
